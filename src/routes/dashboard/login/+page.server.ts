...
export const actions: Actions = {
	login: async ({ request, platform, cookies, getClientAddress, locals }) => {
		const db = getDb(platform);
		const form = await request.formData();
		const email = String(form.get('email') ?? '').trim();
		const password = String(form.get('password') ?? '');
		const staff = await authenticate(db, email, password);
		if (!staff) return fail(400, { message: t(locals.locale, 'lg.errBadCreds') });
	...
	bootstrap: async ({ request, platform, cookies, getClientAddress, locals }) => {
		const db = getDb(platform);
		if ((await countStaff(db)) > 0) return fail(400, { message: t(locals.locale, 'lg.errExists') });
		const form = await request.formData();
		const email = String(form.get('email') ?? '').trim();
		const password = String(form.get('password') ?? '');
		const name = String(form.get('name') ?? '').trim() || '管理者';
		if (!email || password.length < 8) {
			return fail(400, { message: t(locals.locale, 'lg.errWeak') });
		}
